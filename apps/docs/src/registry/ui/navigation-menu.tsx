  ...
  return (
    <NavigationMenuPrimitive.Icon aria-hidden="true">
      <svg
        ...
        class="relative top-px ml-1 size-3 transition duration-200 group-ui-expanded/trigger:rotate-180 group-data-[orientation=vertical]/menu:-rotate-90 group-ui-expanded/trigger:group-data-[orientation=vertical]/menu:rotate-90"
      >
        <path d="M6 9l6 6l6 -6" />
      </svg>
...
const NavigationMenuViewport = <T extends ValidComponent = "li">(
  ...
  return (
    <NavigationMenuPrimitive.Viewport
      class={cn(
        // base settings
        "pointer-events-none z-1000 flex h-(--kb-navigation-menu-viewport-height) w-(--kb-navigation-menu-viewport-width) origin-(--kb-menu-content-transform-origin) items-center justify-center overflow-x-clip overflow-y-visible rounded-md border bg-popover opacity-0 shadow-lg ui-expanded:pointer-events-auto data-[orientation=vertical]:overflow-y-clip data-[orientation=vertical]:overflow-x-visible ui-expanded:rounded-md",
        // animate
        "animate-content-hide transition-[width,height] duration-200 ease-in ui-expanded:animate-content-show ui-expanded:opacity-100 ui-expanded:ease-out",
        local.class
      )}
      {...others}
    />
  ...
  return (
    <NavigationMenuPrimitive.Portal>
      <NavigationMenuPrimitive.Content
        class={cn(
          // base settings
          "pointer-events-none absolute left-0 top-0 box-border p-4 focus:outline-hidden ui-expanded:pointer-events-auto",
