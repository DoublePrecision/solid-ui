...
When running `bunx @doubleprecision/solidui-cli@latest init`, your tailwind config for content will be overwritten. To fix this, change the `module.exports` to `export default` and the `content` section with the code below to your `tailwind.config.mjs` file:
...
```bash
bunx @doubleprecision/solidui-cli@latest add button
```
